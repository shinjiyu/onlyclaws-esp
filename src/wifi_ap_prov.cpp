...
String pageHtml() {
  ...
  page += F(
      ...
      "<title>ePaper WiFi 配置</title><style>"
...
void handleNotFound() {
  // Captive portal: any unknown host/path -> setup page.
  sendPortal();
}
}  // namespace
...
bool wifiApProvision(uint32_t timeoutMs) {
  ...
  statusMsg = "";
  apSsid = String("EPD-Setup-") + String(EPD_DEVICE_ID).substring(8);
  ...
  server.on("/generate_204", HTTP_GET, sendRedirectRoot);          // Android
  server.on("/gen_204", HTTP_GET, sendRedirectRoot);
  server.on("/hotspot-detect.html", HTTP_GET, sendPortal);        // iOS
  server.on("/library/test/success.html", HTTP_GET, sendPortal);  // iOS
  server.on("/ncsi.txt", HTTP_GET, sendRedirectRoot);             // Windows
